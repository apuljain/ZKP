					    ...
					    vector<Integer> &R)
{
	...
	#ifdef DEBUG
	cout<<"g2: "<<generator[identity]->value<<" v2: "<<vi[identity]<<endl;
	cout<<"\n===ti values===\n";
	PrintVector(ti_values);
	#endif

	//convert into string to be used in hash function	
	string input_string = ComputeHashString(_generators, _public_keys,
						ti_values);	
